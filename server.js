...
app.post('/posts', (req, res) => {
    ...
    //console.log(title, content); // Check if data is received
    const user = getCurrentUser(req);
    if (user){
...
app.post('/delete/:id', isAuthenticated, (req, res) => {
    ...
    if (post && post.username === user.username) {
        ...
        // User is not the owner or post does not exist
        res.status(403).send({ success: false, message: 'Cannot delete post: You are not the owner or the post does not exist.' });
    }
});
...
function addUser(username) {
    ...
    const newUser  = {
        id: users.length +1,
        username,
        avatar_url: generateAvatar(username.charAt(0)),
        memberSince: new Date().toISOString().slice(0,16).replace('T', ' ')
    };
    users.push(newUser);
    return newUser;
}
...
function registerUser(req, res) {
    ...
    if (!username) {
        res.redirect('/register?error=Missing username');
        return;
    }
...
function handleAvatar(req, res) {
    ...
    if(user){
        ...
        res.status(404).send('User not found. No avatar available.');
    }
...
function getCurrentUser(req) {
    ...
    if (!req.session.userId) {
        ...
        return null;  // No user ID in the session, return null or handle appropriately
    }
